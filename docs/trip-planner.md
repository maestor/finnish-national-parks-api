...
Request admission and provider reservations use the shared libSQL/Turso database so separate API instances use the same counters. Suggestions allow 30 requests per client per minute; route and nearby searches allow 5 requests per client per minute. The provider-wide daily ceiling is configured with `GEOAPIFY_DAILY_REQUEST_LIMIT` and defaults to 3,000 credits. Each actual uncached or new in-flight provider attempt reserves one unit for geocoding/suggestions or five units for routing immediately before the upstream call; cache hits and in-flight followers reserve zero. Public trip reads bypass the per-client request bucket, while cold multi-leg routes remain constrained by the shared daily provider ceiling. The routing reservation is an internal conservative work-unit cap, not a claim about the provider's exact billing maximum; confirm the deployed subscription and quota policy before production.

Requests above the 16 KiB planner JSON body limit return `413`. Exhausted client or provider budgets return `429` with `Retry-After`; unavailable budget storage fails closed with `503` before new provider work. The frontend proxy counts streamed request bytes before buffering and supplies the API with a server-issued opaque client ID for admitted planner requests. Public trip reads do not receive that identity because they do not consume the per-client request bucket. Confirm the daily unit ceiling and any platform-level rate rules against the actual provider subscription before production deployment.
