...
- Published year-review shares return stable snapshot payloads instead of live derived responses; when a published story includes `photo-highlight.featuredImage`, preview and share reads resolve fresh presigned `fullUrl` and `thumbUrl` values from the frozen image keys at response time.
