...
- `GET /api/home-summary` returns cache-friendly home summary data including seasonal visit counts, `progressByType` visibility flags, aggregated `progressByCategory`, up to 5 `mostVisitedParks`, `recentVisits`, `latestVisitEntries`, and `latestTrips`, without notes, routes, or images. `latestTrips` includes each trip's `name`, `slug`, and derived `startDate`.
...
- `GET /api/trips` returns named trips with derived `dateRange`, `visitCount`, persisted `slug`, optional `startingPoint`, and publication metadata; migration tests prove existing trips remain published and new trips are unlisted.
...
- `GET /api/trip-stories` returns only published trip summaries, with derived dates, places, seasons, years, image counts, ordering, and `private, no-store` caching.
- `GET /api/trips/:id` returns one trip with a merged itinerary that includes both park visits and non-park trip stops in shared `tripStopOrder`, including optional trip-stop `displayName`, optional visit `location` overrides for admin workflows, and `images: VisitImage[]` on trip stops.
- `GET /api/visits-timeline` returns the lightweight `/kaynnit` timeline dataset with `imageCount`, `trip: { id, name, published, slug } | null`, `tripStopOrder: number | null`, and pre-resolved park `typeLabel` values.
...
- Published date-range-review shares return stable snapshot payloads instead of live derived responses; when a published story includes visit-based `featuredImage` fields for photo-highlight, `new-parks`, `revisited-parks`, or `trip-summary` cards, preview and share reads resolve fresh presigned `fullUrl` and `thumbUrl` values from the frozen image keys at response time.
- Year-review preview, publish, and unpublish stay admin-session protected, while published-share reads require the API key for non-localhost callers and remain suitable for server-rendered frontend share pages.
- Published year-review shares return stable snapshot payloads instead of live derived responses; when a published story includes visit-based `featuredImage` fields for milestone, photo-highlight, trip-highlight, or `new-parks` cards, preview and share reads resolve fresh presigned `fullUrl` and `thumbUrl` values from the frozen image keys at response time.
- `POST /api/trip-planner/suggestions` returns up to three Geoapify-backed place suggestions with labels and coordinates for origin/destination pickers across Finland, Sweden, and Norway.
