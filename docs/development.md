...
- `GET /api/trips/:id` returns one trip with a merged itinerary ordered by `tripStopOrder`, combining park visits and non-park trip stops for route-printing or other trip-detail UIs. This detail route uses `private, no-store`.
...
- `POST /api/trips/:id/stops`, `PATCH /api/trip-stops/:id`, and `DELETE /api/trip-stops/:id` are admin-session write routes for non-park trip stops. Stops use the same labeled coordinate shape as trip `startingPoint`, carry an optional note, and participate in the same shared `tripStopOrder` sequence as park visits.
