...
- `GET /api/trips/slug/:slug` returns one page-ready trip detail payload by public slug, including derived `imageCount` / `stopCount`, itinerary visit park `markerPoint` / `typeLabel` / per-visit `imageCount`, and optional round-trip `LineString` geometry when the trip has a starting point and at least two visit waypoints. If route generation is unavailable or a leg cannot be routed, the endpoint still returns the trip detail with `route: null`. This detail route uses `private, no-store`.
