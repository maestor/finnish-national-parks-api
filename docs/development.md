...
- `GET /api/trips/slug/:slug` returns one page-ready trip detail payload by public slug, including derived `imageCount` / `stopCount`, itinerary visit park `markerPoint` / `typeLabel` / per-visit `imageCount`, and a route state shaped as `route: { success, error, data }`. Missing route prerequisites return `route.success: true` with `route.data: null`, while actual routing failures keep the trip payload at `200` and expose the route problem inside `route.error`, including failed-leg details when available. This detail route uses `private, no-store`.
- `GET /api/trips/:id` returns one trip with a merged itinerary ordered by `tripStopOrder`, combining park visits and non-park trip stops for route-printing or other trip-detail UIs. Trip stops expose their own `visitedOn` date in that merged response. This detail route uses `private, no-store`.
...
- `GET /api/visits` returns flat visit resources with their parent park reference and `trip: { id, name, slug } | null`.
- `GET /api/visits/:id` returns one visit with its parent park reference and `trip: { id, name, slug } | null`.
...
- `POST /api/parks/:slug/visits` and `PATCH /api/visits/:id` accept `tripId`, with `null` clearing an existing trip assignment, plus optional `tripStopOrder` for explicit stop ordering inside a named trip.
