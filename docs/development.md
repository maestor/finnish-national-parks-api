...
- `GET /api/trips` returns named trips with derived `dateRange`, `visitCount`, persisted `slug`, optional `startingPoint`, and additive `publication` metadata. Existing rows are backfilled as `published`, while new trips are `unlisted`; the `trip_cover_images` table stores a future selected cover reference without copying image objects. The `dateRange` expands to include qualifying trip stops as well as park visits.
...
- `GET /api/trip-stories` returns lightweight summaries for published trips only, deriving places, actual years, seasons, counts, ordering, and an explicitly selected cover image. It uses `private, no-store` because cover URLs are presigned; direct slug detail remains available for unlisted trips.
...
- `GET /api/visits-timeline` returns the lightweight visits timeline dataset for the public `/kaynnit` screen, including `imageCount`, `trip: { id, name, published, slug } | null`, `tripStopOrder: number | null`, and a resolved park `typeLabel`.
