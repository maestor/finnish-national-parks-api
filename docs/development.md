...
- `GET /api/date-range-review/preview?name=<name>&startDate=<yyyy-mm-dd>&endDate=<yyyy-mm-dd>` returns an admin-session-protected generated preview for one named date-range review. The backend derives the story automatically from existing visit timeline and trip data, limits ranges to 184 days, requires at least 3 visits in-range, and emits intro, photo-highlight, optional new-national-parks, optional revisited-parks, per-trip summary cards with their included park visits, and an optional trailing `other-visits` card for any remaining in-range visits not already covered elsewhere in the story. If the overview name has already been published for a different date range, the endpoint returns `409` instead of reusing that name for another period.
- `POST /api/date-range-review/publish` snapshots the current named date-range review into one published share token keyed by the normalized overview name and returns the token plus the intended frontend share path. Once a name has been published, that same overview name stays bound to its original `startDate` and `endDate`; trying to publish the same name for a different period returns `409`.
...
- `POST /api/trip-planner/search` first resolves exact known park and trail names from the local catalog, falls back to Geoapify geocoding only for unmatched free-text endpoints, fetches a real Geoapify driving route, filters visible parks by a configurable corridor distance, and returns list-ready results with visited summaries plus a map-ready route `LineString`, backend-provided route and park bounding boxes, and top-level `maxDistanceKm` / `defaultDistanceKm` filter metadata. On longer trips, the first 30 km from the origin uses a stricter start-zone filter so dense departure areas do not flood the list. Route failures return `422` with failed-leg details instead of a silent success payload.
- `POST /api/trip-planner/nearby` first resolves exact known park and trail names from the local catalog, falls back to Geoapify geocoding only for unmatched free-text origins, and returns list-ready results with visited summaries plus a backend-provided `searchArea` bounding box and top-level `maxDistanceKm` / `defaultDistanceKm` filter metadata for map rendering without route geometry.
