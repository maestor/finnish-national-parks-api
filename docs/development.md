...
- `GET /api/trips` returns named trips with derived `dateRange`, `visitCount`, persisted `slug`, optional `startingPoint`, and the same shared-cache behavior as the other public summary datasets.
...
- Trip-stop writes require the trip to already contain at least one visit, and the stop `visitedOn` date must stay inside the trip's visit-derived date range, including the start and end dates.
