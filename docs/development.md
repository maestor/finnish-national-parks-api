...
- `GET /api/year-review/shares/{shareId}` returns the published year-review snapshot for trusted API-key callers such as the frontend share page. The snapshot can now include an optional `photo-highlight.featuredImage`; the stored share freezes only stable image keys and dimensions, while each read resolves fresh presigned `fullUrl` and `thumbUrl` values so the response stays usable without storing expired URLs in the snapshot itself.
