...
- `GET /api/year-review/shares/{shareId}` returns the published year-review snapshot for trusted API-key callers such as the frontend share page. The payload intentionally stays image-agnostic so the share does not depend on expiring presigned visit-image URLs.
