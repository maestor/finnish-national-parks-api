...
- `GET /api/visits-timeline` returns the lightweight visits timeline dataset for the public `/kaynnit` screen, including `imageCount`, `trip: { id, name } | null`, `tripStopOrder: number | null`, and a resolved park `typeLabel`.
