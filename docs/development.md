...
- `GET /api/trips/slug/:slug` returns one page-ready trip detail payload by public slug, including derived `imageCount` / `stopCount`, itinerary visit park `markerPoint` / `typeLabel` / per-visit `imageCount`, trip-stop image counts, and an empty initial route state. Visit itinerary entries now include `excludeFromRoute: boolean` and optional `location: { lat, lon } | null`; when `location` is present, the public trip itinerary `park.markerPoint` and route-building waypoint use that visit-specific point instead of the park default marker. It never calls the trip planner and uses `private, no-store`.
- `GET /api/trips/slug/:slug/route` loads or builds the route separately. A durable fingerprint cache reuses unchanged routes and is invalidated only by route-affecting starting-point, visit, or stop changes; descriptive text and image changes do not invalidate it. Missing prerequisites return a successful empty state, and planner failures remain in the `200` response's route error state. The response uses `private, no-store`.
- `GET /api/trips/:id` returns one trip with a merged itinerary ordered by `tripStopOrder`, combining park visits and non-park trip stops for route-printing or other trip-detail UIs. Trip stops expose their own `visitedOn` date, optional custom `displayName`, plus `images: VisitImage[]` in that merged response, and visit entries expose `excludeFromRoute: boolean` plus optional `location: { lat, lon } | null` for admin route control. This detail route uses `private, no-store`.
