...
- `GET /api/trips/slug/:slug` returns one page-ready trip detail payload by public slug, including derived `imageCount` / `stopCount`, itinerary visit park `markerPoint` / `typeLabel` / per-visit `imageCount`, and a route state shaped as `route: { success, error, data }`. Visit itinerary entries now include `excludeFromRoute: boolean`; excluded visits still stay in the itinerary payload but are omitted from route building and route distance. Missing route prerequisites return `route.success: true` with `route.data: null`, while actual routing failures keep the trip payload at `200` and expose the route problem inside `route.error`, including failed-leg details when available. This detail route uses `private, no-store`.
- `GET /api/trips/:id` returns one trip with a merged itinerary ordered by `tripStopOrder`, combining park visits and non-park trip stops for route-printing or other trip-detail UIs. Trip stops expose their own `visitedOn` date in that merged response, and visit entries expose `excludeFromRoute: boolean` for admin route control. This detail route uses `private, no-store`.
...
- `GET /api/visits` returns flat visit resources with their parent park reference, `trip: { id, name, slug } | null`, and `excludeFromRoute: boolean`.
- `GET /api/visits/:id` returns one visit with its parent park reference, `trip: { id, name, slug } | null`, and `excludeFromRoute: boolean`.
