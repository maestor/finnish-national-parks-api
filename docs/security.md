...
- Date-range-review preview and publish routes (`GET /api/date-range-review/preview`, `POST /api/date-range-review/publish`, and `DELETE /api/date-range-review/publish`) stay on the admin-session side, while `GET /api/date-range-review/shares/:shareId` stays on the API-key-protected frontend-read side for server-rendered share pages.
