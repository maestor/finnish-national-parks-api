...
- `GET /api/trips` returns named trips with derived `dateRange`, `visitCount`, persisted `slug`, optional `startingPoint`, and additive `publication` metadata. Existing trips migrate as `published`; newly created trips start `unlisted` until the publication mutation is added.
...
- `GET /api/trip-stories` returns only published trip story summaries with derived dates, places, seasons, years, image counts, and an explicitly selected cover image. It uses `Cache-Control: private, no-store` because cover URLs are presigned; direct slug detail remains available for unlisted trips.
...
- `GET /api/visits` returns flat visit resources with their parent park reference, `trip: { id, name, published, slug } | null`, `tripStopOrder: number | null`, `excludeFromRoute: boolean`, and optional `location: { lat, lon } | null`.
...
- Trip-stop images use the same two-step direct upload flow: `POST /api/trip-stops/:id/images/upload-url`, upload the file to R2 with the returned `PUT` URL, then call `POST /api/trip-stops/:id/images/complete`.
