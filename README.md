...
- `npm run media:backfill-derivatives -- [--apply] [--batch-size <1-100>] [--cursor <opaque-cursor>]` reports one bounded batch of legacy rows whose full and thumbnail keys are identical. It is dry-run by default; after reviewing its counts and failures, use `--apply` to create deterministic server-owned JPEG derivatives and persist their metadata. It retains all source objects. Only resume an apply run with its `nextCursor`; a dry-run's `previewCursor` is informational and must not be used for writes.
