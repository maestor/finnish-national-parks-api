...
- `GET /api/trips/:id` returns one trip with a merged itinerary ordered by `tripStopOrder`, combining park visits and non-park trip stops. Trip stops now include their own `visitedOn` date in the itinerary response, and visit entries include `excludeFromRoute: boolean` plus optional `location: { lat, lon } | null` for admin-side route control. This detail route uses `Cache-Control: private, no-store`.
