...
- `GET /api/date-range-review/preview?name=<name>&startDate=<yyyy-mm-dd>&endDate=<yyyy-mm-dd>` is an admin-session-protected generated preview for a named date-range review. It supports spans up to 184 days, requires at least 3 visits inside the requested range, includes intro, photo-highlight, optional new-national-parks, optional revisited-parks, and per-trip summary cards, and reports any current published share token for that normalized overview name.
- `POST /api/date-range-review/publish` is an admin-session-protected publish action that snapshots the current named date-range review into one tokenized share keyed by the normalized overview name and returns the share token plus the intended frontend share URL.
