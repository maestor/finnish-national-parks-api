...
- `GET /api/visits-timeline` returns the lightweight timeline dataset for `/kaynnit`, with each visit including `id`, `visitedOn`, `createdAt`, `route`, `imageCount`, `trip: { id, name } | null`, and a resolved park `typeLabel`.
...
- `POST /api/parks/:slug/visits`, `PATCH /api/visits/:id`, and `DELETE /api/visits/:id` are admin-session write routes for owned visit data, and visit create/update accepts `tripId` with `null` clearing an assignment.
