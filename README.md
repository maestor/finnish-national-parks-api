...
- `PATCH /api/parks/:slug` updates the admin-editable park fields (`name`, `slug`, `locationLabel`, `postalOffice`, `postalCode`, `areaKm2`, `establishmentYear`, `parkUrl`, `displayTypeName`, optional nullable `markerPoint`) and auto-generates a slug from `name` when `slug` is omitted. `markerPoint: null` clears an override and restores the imported/default marker.
