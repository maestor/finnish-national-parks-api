...
- `POST /api/trips/:id/stops`, `PATCH /api/trip-stops/:id`, and `DELETE /api/trip-stops/:id` are admin-session write routes for non-park trip stops. Stops use the same labeled coordinate shape as trip `startingPoint`, require their own `visitedOn` date, can include an optional note, and share the same `tripStopOrder` sequence as park visits so route-relevant stops can sit between visits. Stop writes require the trip to already contain at least one visit, and each stop date must stay inside the trip's visit-derived date range, inclusive.
